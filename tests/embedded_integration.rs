...
async fn test_embedded_backend_basic_workflow() {
    ...
    let backend = EmbeddedModelBackend::with_variant_and_path(
        ModelVariant::detect(),
        model_path
...
async fn test_embedded_command_generation() {
    let model_path = test_model_path();
    let mut backend = EmbeddedModelBackend::with_variant_and_path(
        ModelVariant::detect(),
        model_path
    ).expect("Failed to create embedded backend");
...
async fn test_variant_selection() {
    ...
    let cpu_backend = EmbeddedModelBackend::with_variant_and_path(
        ModelVariant::CPU,
        model_path.clone()
    ...
    // Test auto-detection
    let auto_backend = EmbeddedModelBackend::with_variant_and_path(
        ModelVariant::detect(),
        model_path
...
async fn test_embedded_performance() {
    ...
    assert!(load_duration < Duration::from_secs(3), 
            "First inference took too long: {:?}", load_duration);
    ...
    assert!(inference_duration < Duration::from_secs(2),
            "Inference took too long: {:?}", inference_duration);
}
...
async fn test_lazy_loading() {
    ...
    let request = CommandRequest::new("test", ShellType::Bash);
    let result = backend.generate_command(&request).await;
    assert!(result.is_ok(), "First inference should trigger loading and succeed");
    ...
    assert!(duration < Duration::from_secs(2), "Second inference should be faster");
}
